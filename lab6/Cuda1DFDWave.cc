...
int main(int argc, char* argv[]) {
...

  {
    ...
    // allocate a single array of size 3 * numberOfNodes, and zero out memory
    float *dev_Data;
    cudaMalloc((void**) &dev_Data, 3*numberOfNodes * sizeof(float));
    cudaMemset(dev_Data, 0, 3*numberOfNodes * sizeof(float));
    ...
    for (size_t timestepIndex = 0; timestepIndex < numberOfTimesteps;
        ...
        int oldStart = numberOfNodes * ((timestepIndex - 1) % 3);
        int currentStart = numberOfNodes * ((timestepIndex) % 3);
        int newStart = numberOfNodes * ((timestepIndex + 1) % 3);
        
        // Call helper function to call the kernel
        kernelCall(dev_Data, oldStart, currentStart, newStart, courantSquared,
             numberOfNodes, blocks, threadsPerBlock);
        ...
        for all times t */
        cudaMemcpy(&dev_Data[newStart], &left_boundary_value, sizeof(float),
             cudaMemcpyHostToDevice);
        cudaMemcpy(&dev_Data[newStart + numberOfNodes - 1], 0, sizeof(float),
             cudaMemcpyHostToDevice);
        ...
        if (CUDATEST_WRITE_ENABLED == true && numberOfOutputFiles > 0 &&
            ...
            cudaMemcpy(file_output, &dev_Data[newStart], numberOfNodes * sizeof(float), cudaMemcpyDeviceToHost);
